		 ...
		 double zerrsum, double zsum, double zavgpgp, int ndim)
{
  ...
  fprintf(fd, " %+20s %dD%d %20.13le %20.13le %20.13le %20.13le %20.13le %s\n",
         name, ndim, iopt, zminval, zmaxval, zmaxerr, zavgpgp, zrelerr, "");
}
...
void validate_1d(const char *name, double * v_ref, double * v_field, int nlon, int ngptot, int nblocks)
{
  ...
  typedef double (*vla_1d)[nlon];
  vla_1d field = (vla_1d) v_field;
  vla_1d reference = (vla_1d) v_ref;
...
void validate_2d(const char *name, double *v_ref, double *v_field, int nlon, int nlev, int ngptot, int nblocks)
{
  ...
  int b, bsize, jl, jk;
  double zminval, zmaxval, zdiff, zmaxerr, zerrsum, zsum, zrelerr, zavgpgp;
  typedef double (*vla_2d)[nlev][nlon];
  vla_2d field = (vla_2d) v_field;
  vla_2d reference = (vla_2d) v_ref;
    ...
    int nlev, int nclv, int ngptot, int nblocks)
{
  ...
  double zminval, zmaxval, zdiff, zmaxerr, zerrsum, zsum, zrelerr, zavgpgp;
  typedef double (*vla_3d)[nclv][nlev][nlon];
  vla_3d field = (vla_3d) v_field;
  vla_3d reference = (vla_3d) v_ref;
		     ...
		     double *tend_loc_a, double *tend_loc_q, double *tend_loc_t, double *tend_loc_cld)
{
  ...
  fd = fopen("load_reference.bin", "rb");
  if (fd) {
    fread(ref_plude, sizeof(double), nblocks*nlev*nproma, fd);
    fread(ref_pcovptot, sizeof(double), nblocks*nlev*nproma, fd);
    fread(ref_prainfrac_toprfz, sizeof(double), nblocks*nproma, fd);
    fread(ref_pfsqlf, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fread(ref_pfsqif, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fread(ref_pfcqlng, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fread(ref_pfcqnng, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fread(ref_pfsqrf, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fread(ref_pfsqsf, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fread(ref_pfcqrng, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fread(ref_pfcqsng, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fread(ref_pfsqltur, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fread(ref_pfsqitur, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fread(ref_pfplsl, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fread(ref_pfplsn, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fread(ref_pfhpsl, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fread(ref_pfhpsn, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fread(ref_tend_loc_a, sizeof(double), nblocks*nlev*nproma, fd);
    fread(ref_tend_loc_q, sizeof(double), nblocks*nlev*nproma, fd);
    fread(ref_tend_loc_t, sizeof(double), nblocks*nlev*nproma, fd);
    fread(ref_tend_loc_cld, sizeof(double), nblocks*nclv*nlev*nproma, fd);
    fclose(fd);
  }

  /*fd = fopen("load_reference.bin", "wb");
  if (fd) {
    fwrite(ref_plude, sizeof(double), nblocks*nlev*nproma, fd);
    fwrite(ref_pcovptot, sizeof(double), nblocks*nlev*nproma, fd);
    fwrite(ref_prainfrac_toprfz, sizeof(double), nblocks*nproma, fd);
    fwrite(ref_pfsqlf, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fwrite(ref_pfsqif, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fwrite(ref_pfcqlng, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fwrite(ref_pfcqnng, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fwrite(ref_pfsqrf, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fwrite(ref_pfsqsf, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fwrite(ref_pfcqrng, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fwrite(ref_pfcqsng, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fwrite(ref_pfsqltur, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fwrite(ref_pfsqitur, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fwrite(ref_pfplsl, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fwrite(ref_pfplsn, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fwrite(ref_pfhpsl, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fwrite(ref_pfhpsn, sizeof(double), nblocks*(nlev+1)*nproma, fd);
    fwrite(ref_tend_loc_a, sizeof(double), nblocks*nlev*nproma, fd);
    fwrite(ref_tend_loc_q, sizeof(double), nblocks*nlev*nproma, fd);
    fwrite(ref_tend_loc_t, sizeof(double), nblocks*nlev*nproma, fd);
    fwrite(ref_tend_loc_cld, sizeof(double), nblocks*nclv*nlev*nproma, fd);
    fclose(fd);
  }*/

  fd = fopen("error.txt", "w");

  //fprintf(fd, " %+20s %s %+20s %+20s %+20s %+20s %+20s\n",
