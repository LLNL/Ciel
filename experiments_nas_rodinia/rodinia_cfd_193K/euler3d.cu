...
void dump(const char* data_file_name, float* variables, int nel, int nelr)
{
	double baseline_origin_energy = 0.0;
	double threshold = 1.0e30;
	...
	else if (file_name_string.find("fvcorr.domn.193K") != std::string::npos) {
		baseline_origin_energy = 710622.31991663738;
		threshold = 0.19;
	}
	...
	if (PRINT_ALL_DATA) {
		{
			//std::ofstream file("euler3d_density.txt");
		...
		{
			//std::ofstream file("euler3d_momentum.txt");
		...
		{
			//std::ofstream file("euler3d_density_energy.txt");
	...
	double energy_diff = abs(baseline_origin_energy - total_energy);
