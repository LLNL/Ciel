...
void dump(const char* data_file_name, double* variables, int nel, int nelr)
{
	double baseline_origin_energy = 0.0;
	std::string file_name_string = data_file_name;
	if (file_name_string.find("fvcorr.domn.097K") != std::string::npos) {
		baseline_origin_energy = 361489.40625874756;
	}
	...
	if (PRINT_ALL_DATA) {
		{
			//std::ofstream file("euler3d_double_density.txt");
			file << "density " << nel << " " << nelr << std::endl;
			for(int i = 0; i < nel; i++) file << (float)h_variables[i + VAR_DENSITY*nelr] << std::endl;
		}


		{
			//std::ofstream file("euler3d_double_momentum.txt");
			file << "momentum " << nel << " " << nelr << std::endl;
			for(int i = 0; i < nel; i++)
			{
				for(int j = 0; j != NDIM; j++)
					file << (float)h_variables[i + (VAR_MOMENTUM+j)*nelr] << " ";
				file << std::endl;
			}
		}
		
		{
			//std::ofstream file("euler3d_double_density_energy.txt");
			file << "energy " << nel << " " << nelr << std::endl;
			for(int i = 0; i < nel; i++) file << (float)h_variables[i + VAR_DENSITY_ENERGY*nelr] << std::endl;
		}
	}

	double total_energy = 0;
	for(int i = 0; i < nel; i++)
		total_energy += h_variables[i + VAR_DENSITY_ENERGY*nelr];
	printf("Total energy diff %.17g\n", abs(baseline_origin_energy - total_energy));
...
int main(int argc, char** argv)
{
	...
	dump(data_file_name, variables, nel, nelr);
