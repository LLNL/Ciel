...
lud_verify(float *m, float *lu, int matrix_dim){
  ...
  float max_dismatch = 0.0f;

  for (i=0; i<matrix_dim; i++){
      for (j=0; j<matrix_dim; j++){
          if ( fabs(m[i*matrix_dim+j]-tmp[i*matrix_dim+j]) > max_dismatch)
            max_dismatch = fabs(m[i*matrix_dim+j]-tmp[i*matrix_dim+j]);
          //if ( fabs(m[i*matrix_dim+j]-tmp[i*matrix_dim+j]) > 0.0001)
          //  printf("ERROR: dismatch at (%d, %d): (o)%f (n)%f\n", i, j, m[i*matrix_dim+j], tmp[i*matrix_dim+j]);
      }
  }
  //printf("max dismatch: %.17g\n", max_dismatch);
  if (max_dismatch > 1.2e-5f)
